...
function bindDirective (bside, el, bindings, directive) {
	...
	if (!bside.scope.hasOwnProperty(key)) {
		bindAccessors(bside, key, binding);
	}
}

function bindAccessors (bside, key, binding) {
	Object.defineProperty(bside.scope, key, {
		get: function () {
		...
		set: function (value) {
			...
			binding.directives.forEach(function (directive) {
				...
				directive.update(directive.el, value, directive.argument, directive, bside);
			})
		}
	})
}
...
function parseDirective (attr) {
	...
	var exp = attr.value,
		...
		filters = pipeIndex === -1
					? null : exp.slice(pipeIndex + 1).split('|').map(function (filter) {
						return filter.trim();
					});
	return def
		? {
